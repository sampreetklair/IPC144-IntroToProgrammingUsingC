...
double calculatecost_per_serving(const double* price, const double* numberOfServings, double* result) {
	...
	double cost_per_serving = (*price) / (*numberOfServings);
...
struct ReportData calculateReportData(struct CatFoodInfo cfi) {
	...
	rd.cost_per_serving = calculatecost_per_serving(&rd.productPrice, &rd.servings, &rd.cost_per_serving);
	rd.cost_per_calories = calculateCostPerCal(&rd.productPrice, &totalCalories, &rd.cost_per_calories);
...
void displayReportData(const struct ReportData rd, const int cheapestProductCheck) {
	...
	printf("%07d %10.2lf %10.1lf %10.4lf %9d %8d %8.1lf %7.2lf %7.5lf", rd.skuNumber, rd.productPrice, rd.productWeightPounds, rd.productWeightKilos, rd.productWeightGrams, rd.calories, rd.servings, rd.cost_per_serving, rd.cost_per_calories);
}
...
void start(void)
{
	...
	double cheapestProductPrice = rd[0].cost_per_serving;
	for (i = 0; i < MAX_PRODUCTS; i++)
	{
		if (rd[i].cost_per_serving <= cheapestProductPrice)
		{
			cheapestProductPrice = rd[i].cost_per_serving;
			cheapestIndex = i;
		}
	}
	printf("\n");
