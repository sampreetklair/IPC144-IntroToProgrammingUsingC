...
double lbstokgconversion(const double* lbs, double* result_lbstokg){
...
int lbstogconversion(const double* lbs, int* result_lbstog){
...
void lbsconversion(const double* lbs, double* result_kg, double* result_gm){
	double lbstokg = (*lbs) / LBS_KG_CONSTANT;
	int lbstog = ((*lbs) / LBS_KG_CONSTANT) * 1000;
	if (result_gm != NULL && result_kg != NULL){
		*result_gm = LBS_KG_CONSTANT;
		*result_kg = LBS_KG_CONSTANT;
	}
}
...
struct data_reporting detail_record(struct CatFoodInfo food){
	...
	record.weight_kg = lbstokgconversion(&record.weight, &record.weight_kg);
	record.weight_gm = lbstogconversion(&record.weight, &record.weight_gm);
